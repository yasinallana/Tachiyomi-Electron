...
async function s() {
  const {data} = await axios.get("https://mangakomi.com/manga/?m_orderby=latest");

  const dom = new convertToHtml(data);
  const document = dom.window.document;

  const mangaTiles = document.querySelectorAll('.manga .item-thumb a');
  
  for (const manga of mangaTiles) {
    console.log({man: manga.href})
  }
